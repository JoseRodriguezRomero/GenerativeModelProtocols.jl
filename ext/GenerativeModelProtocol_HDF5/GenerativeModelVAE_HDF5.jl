function GenerativeModelProtocols.load_variational_autoencoder_parameters(file::FileIO.File{FileIO.DataFormat{:HDF5},String};
    ...
    HDF5.h5open(file.filename, "r") do file
        ...
        encoders = Vector{GenerativeModelProtocols.ChainParameters}(undef, length(keys(encoder_group)))
        decoders = Vector{GenerativeModelProtocols.ChainParameters}(undef, length(keys(decoder_group)))

        for i in eachindex(encoders)
            encoder_chain_group = encoder_group["encoder $i"]
            encoders[i] = GenerativeModelProtocols.ChainParameters(read_group_layer_parameters(encoder_chain_group))
        end

        for i in eachindex(decoders)
            decoder_chain_group = decoder_group["decoder $i"]
            decoders[i] = GenerativeModelProtocols.ChainParameters(read_group_layer_parameters(decoder_chain_group))
        end

        return GenerativeModelProtocols.VariationalAutoencoderParameters(encoders, decoders)
    end
end
...
function GenerativeModelProtocols._save(file::FileIO.File{FileIO.DataFormat{:HDF5},String}, 
    ...
    encoder_parameters = GenerativeModelProtocols.variational_autoencoder_parameters(model)
    HDF5.h5open(file.filename, "w") do file
        ...
        function write_encoder_to_file(encoder::Vector)
        ...
        function write_decoder_to_file(decoder::Vector)
        ...
        write_encoder_to_file(encoder_parameters.encoders)
        write_decoder_to_file(encoder_parameters.decoders)
    end
end
