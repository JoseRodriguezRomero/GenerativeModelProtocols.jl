...
\begin{document}
	...
	\begin{tikzpicture}[
		...
		\node[layer, left=of c1, xshift=-3.5cm] (in_proj) {input\_projection};
		...
		\node[above=0.35 of solid_branch] (hkminus1_node) {$h_{k-1}$};
		...
		\draw[->, line width=2pt] (add_t) -- (out_proj) node[pos=0.75, above] {};
		\draw[->, line width=2pt] (out_proj) -- (block_out);
		
		\node[above right=0.0 and 7.0 of add_t.center] (h3_node) {$h_3$};
		\node[above right=0.0 and 3.0 of add_t.center] (hk_node) {$h_k$};
